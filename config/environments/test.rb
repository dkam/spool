...
Rails.application.configure do
  ...
  # The reader/writer split stays ON in test, deliberately.
  #
  # It was briefly turned off here, because a legitimate write on a GET
  # (TicketsController#show marking a ticket read) raised ReadOnlyError despite
  # being wrapped in ApplicationRecord.writing. That was a real symptom with a
  # different cause than it appeared to have: `prevent_writes` is tracked per
  # connection class, the middleware sets it on ActiveRecord::Base, and the wrap
  # was clearing it on ApplicationRecord — a different flag from the one the
  ...
  # reader connection would see an empty database.
end
